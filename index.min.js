...
function wrapper(my) {
    ...
    function go(datasource) {
        !0 === my.metadata && (metadata = path.join(my.root, ".metadata", path.sep)), require("mongodb").MongoClient.connect(my.uri, my.options, function(err, db) {
    ...
    if (meta = !0 === my.metadata ? readMetadata : function(a, b, c) {
        return c();
    }, !my.tar) return go(function(rootDir) {
...
var systemRegex = /^system\./, fs = require("graceful-fs"), path = require("path"), logger, meta, streamListener = function(parser, writer) {
    var collectionNameForDirectory = new RegExp(".*/([^/]+)/?$"), collectionNameForFiles = new RegExp(".*/([^/]+)/[^/]+$");
    return {
    ...
    var dataToWrite = {}, writeAllInCollection = function(collectionName, callback) {
        ...
        0 !== operations.length ? (console.log("writing to collection " + collectionName), 
